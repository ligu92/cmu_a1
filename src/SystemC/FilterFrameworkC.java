...
*
* FilterFrameworkC:  This is a reference to the filter that is connected to the instance filter's input port. This
...
public class FilterFrameworkC extends Thread
{
	// Define filter input and output ports

	private List<PipedInputStream> InputReadPorts = new ArrayList<PipedInputStream>();
	
	protected List<PipedOutputStream> OutputWritePorts = new ArrayList<PipedOutputStream>();
	...
	private List<FilterFrameworkC> InputFilters = new ArrayList<FilterFrameworkC>();
	...
	*
	* Arguments:
	* 	FilterFrameworkC - this is the filter that this filter will connect to.
	*
	* Returns: void
	*
	...
	****************************************************************************/
	...
	void Connect(FilterFrameworkC Filter)
	{
		try
		{
			// Connect this filter's input to the upstream pipe's output stream
			PipedInputStream InputReadPort = new PipedInputStream();
			PipedOutputStream OutputReadPort = new PipedOutputStream();
			InputReadPort.connect(OutputReadPort);
			
			InputReadPorts.add(InputReadPort);
			Filter.OutputWritePorts.add(OutputReadPort);
			InputFilters.add(Filter);
		} // try

		catch( Exception Error )
		{
			System.out.println( "\n" + this.getName() + " FilterFrameworkC error connecting::"+ Error );
	...
	*
	* Arguments:
	* 	List<FilterFrameworkC> - this is a List of  filters that this filter will connect to.
	*
	* Returns: void
	*
	* Exceptions: IOException
	...
	****************************************************************************/
	...
	void Connect(List<FilterFrameworkC> Filters)
	{
		try
		{
			// Connect this filter's input to the upstream pipe's output stream
			for (FilterFrameworkC filter : Filters) {
				PipedInputStream InputReadPort = new PipedInputStream();
				PipedOutputStream OutputReadPort = new PipedOutputStream();
	...
	* Purpose: This method reads data from the ith input port one byte at a time.
	*
	* Arguments: void
	*
	...
	****************************************************************************/
	...
	byte ReadFilterInputPort(int i) throws EndOfStreamException
	{
		byte datum = 0;
		...
		* broken.
		***********************************************************************/
		// get the ith input read port
		PipedInputStream InputReadPort = InputReadPorts.get(i);
		try
		{
			while (InputReadPort.available()==0 )
			{
				if (EndOfInputStream(i))
				{
	...
	****************************************************************************/
	...
	{
		for (PipedOutputStream OutputWritePort : OutputWritePorts) 
		{ 
			try
			{
	...
	****************************************************************************/
	...
	{
		FilterFrameworkC InputFilter = InputFilters.get(i);
	...
	****************************************************************************/
	...
	void ClosePorts()
	{
		try
		{
			for (PipedInputStream InputReadPort : InputReadPorts) 
			{
				InputReadPort.close();
			}
			for (PipedOutputStream OutputWritePort : OutputWritePorts) 
